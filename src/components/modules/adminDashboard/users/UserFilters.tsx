    ...
    <div className="bg-white dark:bg-slate-900 p-6 rounded-[40px] shadow-sm border border-slate-100 dark:border-slate-800 mb-8">
      <div className="flex flex-col xl:flex-row gap-6 items-end">
        ...
        <div className="flex-1 w-full space-y-2">
          <label className="block text-xs font-black uppercase tracking-widest text-slate-400 ml-1">Search User</label>
        ...
        <div className="flex flex-wrap gap-4 items-center w-full xl:w-auto">
          {/* Role Filter */}
          <div className="flex flex-col gap-2 relative group min-w-[140px]">
            <label className="block text-xs font-black uppercase tracking-widest text-slate-400 ml-1">Filter Role</label>
          ...
          {/* Status Filter */}
          <div className="flex flex-col gap-2 relative group min-w-[140px]">
            <label className="block text-xs font-black uppercase tracking-widest text-slate-400 ml-1">Filter Status</label>
            <div className="relative">
              <select
                value={searchParams.get("status") || "all"}
                onChange={(e) => handleFilterChange("status", e.target.value)}
                className="h-12 px-5 w-full rounded-2xl border border-slate-200 dark:border-slate-700 bg-white dark:bg-slate-900 focus:ring-4 focus:ring-orange-500/10 focus:border-orange-500 transition-all outline-none appearance-none cursor-pointer font-bold text-slate-600 dark:text-slate-400"
              >
          ...
          {/* Clear All Button */}
          {(searchParams.get("searchTerm") || searchParams.get("role") || searchParams.get("status") || searchParams.get("sortOrder")) && (
