...
const DeleteCouponModal = ({ couponId, couponCode }: DeleteCouponModalProps) => {
  ...
  return (
    <Dialog open={isOpen} onOpenChange={setIsOpen}>
      <DialogTrigger asChild>
        <button className="p-2 bg-red-50 text-red-600 hover:bg-red-100 rounded-lg transition-colors cursor-pointer group" title="Delete Coupon">
          ...
          <DialogHeader className="mb-6">
            <div className="flex flex-col items-center text-center">
              ...
              <DialogTitle className="text-2xl font-black tracking-tight text-slate-900 dark:text-white">
                Delete <span className="text-red-500">Coupon?</span>
              </DialogTitle>
              <DialogDescription className="text-slate-500 dark:text-slate-400 font-medium mt-2">
                Are you sure you want to permanently remove <span className="font-bold text-slate-900 dark:text-white">"{couponCode}"</span>? This action cannot be undone.
              </DialogDescription>
            </div>
          </DialogHeader>
